...
if ( isset($_REQUEST['action']) && 'adduser' == $_REQUEST['action'] ) {
    ...
    if ( false !== strpos($_REQUEST[ 'email' ], '@') ) {
        ...
        if ( is_super_admin() ) {
            ...
            wp_redirect( add_query_arg( array('update' => 'enter_email'), 'admin.php?page='.CUN_PAGE_SLUG ) );
            die();
        }
    }

    if ( !$user_details ) {
        wp_redirect( add_query_arg( array('update' => 'does_not_exist'), 'admin.php?page='.CUN_PAGE_SLUG ) );
        die();
    }
    ...
    $redirect = 'admin.php?page='.CUN_PAGE_SLUG;
    $username = $user_details->user_login;
    $user_id = $user_details->ID;
    if ( ( $username != null && !is_super_admin( $user_id ) ) && ( array_key_exists($blog_id, get_blogs_of_user($user_id)) ) ) {
        $redirect = add_query_arg( array('update' => 'addexisting'), 'admin.php?page='.CUN_PAGE_SLUG );
    } else {
            add_existing_user_to_blog( array( 'user_id' => $user_id, 'role' => $_REQUEST[ 'role' ] ) );
            $redirect = add_query_arg( array('update' => 'addnoconfirmation'), 'admin.php?page='.CUN_PAGE_SLUG );
    }
    ...
    if ( ! is_multisite() ) {
        ...
        if ( is_wp_error( $user_id ) ) {
            ...
            if ( current_user_can( 'list_users' ) )
                ...
                $redirect = add_query_arg( 'update', 'add', 'admin.php?page='.CUN_PAGE_SLUG );
            wp_redirect( $redirect );
            die();
        }
    } else {
        ...
        if ( is_wp_error( $user_details[ 'errors' ] ) && !empty( $user_details[ 'errors' ]->errors ) ) {
            ...
            $redirect = add_query_arg( array('update' => 'addnoconfirmation', 'page' => CUN_PAGE_SLUG), 'admin.php' );
            //echo $redirect;
            wp_redirect( $redirect );
            //echo "redirected";
            exit();
...
if ( is_multisite() ) {
    ...
    if ( !is_super_admin() ) {
        echo '<p>' . __( 'Enter the email address of an existing user on this network to invite them to this site.' ) . '</p>';
        $label = __('E-mail');
        $type  = 'email';
    } else {
        echo '<p>' . __( 'Enter the email address or username of an existing user on this network to invite them to this site.' ) . '</p>';
